...
#define HASHMAP_DEFINE_STRING_TYPE(value_type, value_name)                                          \
    ...
    static inline value_type hashmap_get_string_##value_name(                                       \
        hashmap_string_##value_name##_t* map, char* key)                                            \
    {                                                                                               \
        value_type* value_ptr = (value_type*)hashmap_get(map->__internal, key);                     \
        return value_ptr ? *value_ptr : (value_type) { 0 };                                         \
    }                                                                                               \
    static inline value_type* hashmap_getp_string_##value_name(                                     \
        hashmap_string_##value_name##_t* map, char* key)                                            \
    {                                                                                               \
        return (value_type*)hashmap_get(map->__internal, key);                                      \
    }                                                                                               \
    static inline int hashmap_has_string_##value_name(                                              \
        hashmap_string_##value_name##_t* map, char* key)                                            \
    {                                                                                               \
        return hashmap_has(map->__internal, key);                                                   \
    }                                                                                               \
    static inline int hashmap_remove_string_##value_name(                                           \
        hashmap_string_##value_name##_t* map, char* key)                                            \
    {                                                                                               \
        return hashmap_remove(map->__internal, key);                                                \
    }                                                                                               \
    static inline void hashmap_destroy_string_##value_name(                                         \
        hashmap_string_##value_name##_t* map)                                                       \
    {                                                                                               \
